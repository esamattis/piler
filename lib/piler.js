 ...
 * @namespace Piler
 */
var BasePile, CSSManager, CSSPile, JSManager, JSPile, LiveUpdateMixin, PileManager, Q, assetUrlParse, bindFn, cache, debug, defNs, fs, logger, minify, path, production, reserved, serialize, _,
...
BasePile = (function(_super) {
  ...
  BasePile.prototype.addFile = function(filePath, before) {
    ...
    if (__indexOf.call(this.getFilePaths(), filePath) < 0) {
      this.code[!before ? 'push' : 'unshift'](serialize.call({
        type: "file",
        filePath: filePath
      }));
    }
  ...
  BasePile.prototype._computeHash = function() {
    return this.pileHash = serialize.sha1(this.rawPile, 'hex');
  };
  ...
  BasePile.prototype.minify = function(code, options) {
    ...
    if (this.production) {
      return minify.minify(this.ext, code, _.merge({
  ...
  BasePile.prototype.pileUp = function(cb) {
    ...
    self = this;
    return Q.map(this.code, function(codeOb) {
      var d;
      d = Q.defer();
...
PileManager = (function() {
  ...
  PileManager.prototype.pileUp = function(cb) {
    ...
    return Q.map(Object.keys(piles), function(name) {
      var pile;
      pile = piles[name];
...
module.exports.createJSManager = function(settings) {
  if (settings == null) {
    settings = {};
  }
...
module.exports.createCSSManager = function(settings) {
  if (settings == null) {
    settings = {};
  }
  settings.production = production;
